...
describe("Teacher class", () => {
  describe("create new teacher", () => {
    it("on creating a new teacher, it should be created correctly", () => {
      const TEACHER_NAME = "Mark";
      const teacher = new Teacher({ name: TEACHER_NAME });
      expect(teacher).toEqual({ name: TEACHER_NAME, classes: [] });
    });

    it("on creating a new teacher with missing params, it should throw an error", () => {
      expect(() => {
        new Teacher({});
      }).toThrow();
    });
  ...
  describe("Creating Classes and quizes", () => {
    it("when a teacher creates a new class, it should be created and added correctly", () => {
